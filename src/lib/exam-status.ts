...
export function examAvailability(e: ExamWindow, now = Date.now()): Availability {
  ...
  if (opens && now < opens) return { state: "upcoming", label: `เปิด ${formatDateTime(e.opens_at)}` };
  if (closes && now >= closes) return { state: "closed", label: `ปิดแล้ว ${formatDateTime(e.closes_at)}` };
  if (closes) return { state: "open", label: `เปิดถึง ${formatDateTime(e.closes_at)}` };
...
export function fromBangkokLocalInput(v: string | null | undefined): string | null {
...
export function deadlineOf(startedAt: string, limitMinutes: number | null, closesAt: string | null): number | null {
  const byLimit = limitMinutes ? new Date(startedAt).getTime() + limitMinutes * 60_000 : null;
