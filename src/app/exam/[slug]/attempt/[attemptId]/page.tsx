...
export default async function AttemptPage({
  params,
  searchParams,
}: PageProps<"/exam/[slug]/attempt/[attemptId]">) {
  const { slug, attemptId } = await params;
  const sp = await searchParams;
  const { supabase, user } = await requireUser(
    `/exam/${slug}/attempt/${attemptId}`,
  );

  const { data: attempt } = await supabase
    .from("exam_attempts")
    .select(
      "id, exam_id, user_id, started_at, submitted_at, score, passed, exams(id, slug, title, time_limit_minutes, passing_score, closes_at)",
    )
    .eq("id", attemptId)
    .maybeSingle();
  ...
  const [{ data: choices }, { data: answers }, imageUrls] = await Promise.all([
    ...
    supabase
      ...
      .select("question_id, choice_id, text_answer, is_correct")
  ...
  if (attempt.submitted_at) {
    ...
    return (
      <main className="space-y-6">
        ...
        {sp.feedback === "thanks" && (
          <p role="status" className={alert.ok}>
            ขอบคุณสำหรับ feedback ครับ
          </p>
        )}
        {isOwner && !feedback && sp.feedback !== "thanks" && (
          <p
            className={`flex flex-wrap items-center justify-between gap-3 ${alert.warn}`}
          >
            <span>
              ยังไม่ได้ให้ feedback สำหรับข้อสอบครั้งนี้ ใช้เวลาไม่ถึงนาที
            </span>
            <Link
              href={`/exam/${exam.slug}/attempt/${attempt.id}/feedback`}
        ...
        <section className={`${card} flex flex-wrap items-center gap-6`}>
          ...
          {attempt.passed != null && (
            <span
              className={`${attempt.passed ? badge.green : badge.red} px-3 py-1 text-sm`}
            >
              {attempt.passed ? "ผ่าน" : "ไม่ผ่าน"} (เกณฑ์{" "}
              {formatScore(exam.passing_score)})
            </span>
          )}
          <p className="ml-auto text-sm text-ink-2">
            ส่งเมื่อ {formatDateTime(attempt.submitted_at)}
          </p>
        </section>
        ...
        <section>
          ...
          {!reveal && (
            <p className="mt-1 text-sm text-ink-2">ข้อสอบนี้ไม่แสดงเฉลย</p>
          )}
          <ol className="mt-3 space-y-4">
            {runnerQuestions.map((q, i) => {
              ...
              const shown =
                q.kind === "choice"
                  ? q.choices.find((c) => c.id === raw)?.body
                  : raw;
              const expl = questions?.find((x) => x.id === q.id)?.explanation;
              const ok = correctMap[q.id];
              const rv = reviewByQ.get(q.id);
              const fuzzyHit =
                ok === true && q.kind === "text" && (rv?.distance ?? 0) > 0;
              const correctChoice = rv?.correct_choice_id
                ? q.choices.find((c) => c.id === rv.correct_choice_id)
                : null;
              return (
                <li
                  key={q.id}
                  className={`rounded-xl border p-5 text-sm ${ok === true ? "border-mint/40" : ok === false ? "border-danger/40" : "border-line"}`}
                >
                  ...
                  {q.imageUrl && (
                    ...
                    <img
                      src={q.imageUrl}
                      alt=""
                      className="mt-3 max-h-64 w-auto max-w-full rounded-lg border border-line object-contain"
                    />
                  )}
                  {q.kind === "text" && rv?.closest_answer && raw ? (
                    <div className="mt-3">
                      <AnswerDiff answer={raw} expected={rv.closest_answer} />
                    </div>
                  ) : (
                    <p className="mt-2 text-ink-2">
                      ...
                      {shown ? (
                        ...
                        <span className="italic">ไม่ได้ตอบ</span>
                      )}
                    </p>
                  )}
                  ...
                  {expl && (
                    <p className="mt-2 whitespace-pre-line rounded-lg bg-surface-2 p-3 text-ink-2">
        ...
        <div className="flex gap-3">
          <Link href={`/exam/${exam.slug}`} className={btn.secondary}>
            กลับหน้าข้อสอบ
          </Link>
          <Link href="/exam" className={btn.link}>
            ข้อสอบทั้งหมด
          </Link>
        </div>
      </main>
    ...
    return (
      <main className="space-y-4">
        ...
        <p className="text-ink-2">
          ผู้เรียนยังทำข้อสอบนี้ไม่เสร็จ (เริ่มเมื่อ{" "}
    ...
    <main className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-ink-2">
          คำตอบจะถูกบันทึกทันทีที่เลือก สามารถเปลี่ยนคำตอบได้จนกว่าจะกดส่ง
        </p>
      </div>
      {sp.error === "submit" && (
