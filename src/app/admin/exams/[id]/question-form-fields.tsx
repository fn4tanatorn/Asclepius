    ...
    <>
      ...
      {kind === "choice" ? (
        <fieldset className="space-y-2">
          ...
          {Array.from({ length: 5 }).map((_, ci) => (
            <div key={ci} className="flex items-center gap-2">
              ...
              />
              <span className="w-5 text-sm text-ink-2">{LETTERS[ci]}.</span>
              <input
                name="choice_body"
                required={ci < 2}
                placeholder={ci >= 2 ? "(ไม่บังคับ)" : ""}
                className={input}
              />
            </div>
          ))}
        ...
        <label className={label}>
          <span>
            เฉลย (คำตอบที่ยอมรับ 1 บรรทัดต่อ 1 คำตอบ เช่น ภาษาไทย/อังกฤษ/ตัวย่อ)
          </span>
          <textarea
            name="answer_keys"
            rows={4}
          ...
          <span className="block text-xs font-normal text-ink-2">
            ระบบเทียบแบบไม่สนตัวพิมพ์เล็ก-ใหญ่และช่องว่างซ้ำ
          </span>
        </label>
      )}
    </>
