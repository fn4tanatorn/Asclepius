...
export default async function AdminExamPage({ params, searchParams }: PageProps<"/admin/exams/[id]">) {
  ...
  const [{ data: exam }, { data: courses }] = await Promise.all([
    supabase
      .from("exams")
      .select("id, slug, title, description, course_id, time_limit_minutes, passing_score, is_published, questions(id, stem, explanation, points, position, choices(id, body, is_correct, position))")
      .eq("id", id)
      .maybeSingle(),
  ...
  const questions = [...exam.questions]
    ...
    .map((q) => ({ ...q, choices: [...q.choices].sort((a, b) => a.position - b.position) }));
        ...
        <div className="space-y-8">
            ...
            {questions.length === 0 ? (
              ...
              <ol className="mt-3 space-y-3">
                {questions.map((q, i) => {
                  ...
                  const warn = q.choices.length < 2 || q.choices.filter((c) => c.is_correct).length !== 1;
                  return (
                    <li key={q.id} id={`q-${q.id}`} className={card}>
                      <div className="flex items-start gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="mt-2 text-xs text-zinc-500">{Number(q.points)} คะแนน{warn ? " · " : ""}{warn && <span className="text-amber-600">ต้องมีตัวเลือก ≥2 และคำตอบถูก 1 ตัว</span>}</p>
          ...
          <section className={card}>
            ...
            <form action={createQuestion} className="mt-4 space-y-3">
              <input type="hidden" name="exam_id" value={exam.id} />
              <label className={label}><span>โจทย์</span><textarea name="stem" rows={3} required className={input} /></label>
              <fieldset className="space-y-2">
                <legend className="text-sm font-medium">ตัวเลือก (เลือกวงกลมหน้าคำตอบที่ถูก)</legend>
                {Array.from({ length: 5 }).map((_, ci) => (
                  <div key={ci} className="flex items-center gap-2">
                    <input type="radio" name="correct" value={ci} defaultChecked={ci === 0} required aria-label={`ข้อ ${LETTERS[ci]} ถูก`} />
                    <span className="w-5 text-sm text-zinc-500">{LETTERS[ci]}.</span>
