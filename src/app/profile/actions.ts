...
export async function saveLineName(input: {
  lineName: string;
...
export async function updateProfile(formData: FormData) {
  ...
  if (!fullName)
    redirect("/profile?error=" + encodeURIComponent("กรุณากรอกชื่อ"));
  if (!lineName)
    redirect("/profile?error=" + encodeURIComponent("กรุณากรอกชื่อ LINE"));
  const { error } = await supabase
    .from("profiles")
    .update({ full_name: fullName, line_name: lineName })
    .eq("id", user.id);
  if (error)
    redirect("/profile?error=" + encodeURIComponent("บันทึกไม่สำเร็จ"));
  revalidatePath("/", "layout");
