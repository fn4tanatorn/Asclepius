...
export function LineNameModal({ email }: { email: string }) {
  ...
  return (
    <div
      role="dialog"
      aria-modal="true"
      aria-labelledby="line-name-title"
    ...
    >
      ...
      >
        <div>
          <h2 id="line-name-title" className="text-lg font-semibold">
            ชื่อของคุณใน LINE OpenChat
          </h2>
          <p className="mt-1 text-sm text-ink-2">
            เพื่อให้ผู้สอนจับคู่บัญชี{" "}
            <span className="font-medium">{email}</span> กับชื่อในกลุ่ม LINE ได้
            กรอกครั้งเดียว แก้ไขได้ที่หน้าโปรไฟล์
          </p>
        </div>
        ...
        <div className="flex items-center justify-between">
          <button type="button" onClick={snooze} className={btn.link}>
            ไว้ทีหลัง
          </button>
          <button
            type="submit"
